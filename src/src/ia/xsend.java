...
*/
public class xsend extends DefaultInternalAction {
    ...
    @Override protected void checkArguments(Term[] args) throws JasonException {
        ...
        if (args.length < 4 || !args[args.length-1].isString())
            throw JasonException.createWrongArgument(this,"protocol id parameter ('"+args[args.length-1]+"') must be a string!");
    }
    ...
    public Object execute(final TransitionSystem ts, Unifier un, Term[] args) throws Exception {
        ...
        Term ilf  = args[1];
        Term pcnt = args[2];
        String pId = ((StringTerm) args[args.length-1]).getString(); 
        ...
        lastSendWasSynAsk = m.isAsk() && args.length > 4;
        ...
        if (to.isList()) {
            for (Term t: (ListTerm)to) {
                delegateSendToArch(t, ts, m, pId);
            }
        } else {
            delegateSendToArch(to, ts, m, pId);
        }
        
        if (lastSendWasSynAsk && args.length == 6) {
            ...
            if (tto.isNumeric()) {
                ts.getAg().getScheduler().schedule( new Runnable() {
                    public void run() {
                        ...
                        if (intention != null) {
                            ...
                            intention.peek().getUnif().unifies(send.getTerm(4), timeoutAns);
                ...
                throw new JasonException("The 6th parameter of send must be a number (timeout) and not '"+tto+"'!");
            }
        }
    ...
    private void delegateSendToArch(Term to, TransitionSystem ts, Message m, String pId) throws Exception {
        ...
        if (ts.getUserAgArch() instanceof XCAgentArch) {
        	((XCAgentArch)ts.getUserAgArch()).sendMsg(m, pId);
        } else {
